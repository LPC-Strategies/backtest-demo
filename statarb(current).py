...
def get_stock_data():
    """Read stock data from Yahoo Finance for the last 150 trading days"""
    # Define stock tickers
    tickers = ['AAPL', 'AMZN', 'GOOG', 'MSFT']
    
    # Calculate date range (request 200 calendar days to get ~150 trading days)
    end_date = datetime.now()
    start_date = end_date - timedelta(days=200)
    
    print(f"Fetching data from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
    print("(Requesting 200 calendar days to ensure ~150 trading days)")
    ...
    for ticker in tickers:
        try:
            ...
            if not df.empty:
                ...
                print(f"✓ Downloaded {ticker}: {len(df)} trading days of data")
            else:
    ...
    if not data:
        raise ValueError("No data could be downloaded from Yahoo Finance")
