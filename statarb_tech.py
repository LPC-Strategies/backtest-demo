...
def get_top_correlation_pairs(data, top_n=5):
    pairs = find_cointegrated_pairs(data, min_correlation=0.0)
    return pairs[:top_n]

def main():
    print("Reading stock data from tech data folder...")
    data = get_stock_data()
    # Only keep tickers in the top 5 pairs
    top_pairs = get_top_correlation_pairs(data, top_n=5)
    tickers = set()
    for p in top_pairs:
        tickers.add(p[0])
        tickers.add(p[1])
    data = data[list(tickers)]
    print(f"\nRestricting to tickers: {sorted(list(tickers))}")
    print(f"Data shape: {data.shape}")
    # Redefine find_cointegrated_pairs to only return these pairs
    def find_top_pairs_only(data, min_correlation=0.0, lookback=60, cointegration_window=252):
        return [p for p in top_pairs if p[0] in data.columns and p[1] in data.columns]
    global find_cointegrated_pairs
    find_cointegrated_pairs = find_top_pairs_only
    # Run the original strategy
    print("\nRunning strategy on top correlation pairs only...")
    results, positions, z_scores, cointegration_spreads, simple_spreads, trades = backtest_cointegration_statarb(data)
    ...
    print(f"\n=== Top Correlation Pairs Strategy Results ===")
